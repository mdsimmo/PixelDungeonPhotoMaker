...
public class GifSequenceWriter {
	...
	public GifSequenceWriter(ImageOutputStream outputStream, int imageType,
			...
			boolean frameReplace) throws IIOException, IOException {
	...
	/**
	 * creates a gif image from a directory of images and writes the output to the specified file.
	 * 
	 * @param input a directory where the input files can be found
	 * @param filter a filter for the input files 
	 * @param outputFile the file where the created gif should be saved
	 * @param timeBtwFramesMS the time (in ms) between consecutive frames
	 * @param loopContinuous whether the image should loop once finished
	 * @param replace whether each frame should write over the previous frame
	 */
	public static void createGif(File input, FilenameFilter filter,
			File outputFile, int timeBtwFramesMS, boolean loopContinuous,
			boolean replace) {
		if (!input.isDirectory()) {
			System.out.print("incorrect input: must be a directory");
			return;
		}
		...
		try {
			...
			gsw = new GifSequenceWriter(out, BufferedImage.TYPE_INT_ARGB,
					timeBtwFramesMS, loopContinuous, replace);
